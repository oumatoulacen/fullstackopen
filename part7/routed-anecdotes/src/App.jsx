...
const CreateNew = ({ addNew, notify }) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (
      contentField.value.length < 5 ||
      authorField.value.length < 5 ||
      infoField.value.length < 5
    ) {
      notify("All fields must be at least 5 characters long");
      return;
    }
    addNew({
...
const AnecdoteDetails = ({ anecdote, vote }) => {
  if (!anecdote) {
  ...
  return (
    <div>
      ...
      <Routes>
        ...
        <Route
          path="/anecdotes/:id"
          element={<AnecdoteDetails anecdote={anecdote} vote={vote} />}
        />
        <Route
          path="/create"
